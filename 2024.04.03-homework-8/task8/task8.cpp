...
public:
	...
	int** getmatrix();
	int getvertexes();
	int countedgesfromvertex(int n);
	void printedgesfromvertex(int n);
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < n; i++)
	{
		std::cout << graph.countedgesfromvertex(i) << " ";
		graph.printedgesfromvertex(i);
		std::cout << std::endl;
	}
...
int CGraph::getvertexes()
{
	...
	return getVertexesCountFromEdges();
}
int CGraph::countedgesfromvertex(int n)
{
	int count = 0;
	n++;
	for (int i = 0; i < _edges; i++)
...
void  CGraph::printedgesfromvertex(int n)
{
	n++;
	for (int i = 0; i < _edges; i++)
	{
